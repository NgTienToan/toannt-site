    ...
    <div className="mb-14 flex flex-col sm:flex-row w-full">
      <div className={`w-full ${coverImage ? 'sm:w-2/5' : 'sm:w-4/5'}`}>
        <Link href={`/${portfolio ? 'portfolio' : 'blog'}/${convertCanonicalURLToRelative(canonical)}`} >
          <a className="text-3xl text-gray-500 hover:text-blue-300 transition-colors">{title}</a>
        </Link>
        ...
        <Link href={`/${portfolio ? 'portfolio' : 'blog'}/${convertCanonicalURLToRelative(canonical)}`} >
          <a className="text-base mt-2 underline text-gray-500 hover:text-sky-300 transition-colors">Read more</a>
        </Link>
      </div>
